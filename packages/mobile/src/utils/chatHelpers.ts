...
export function formatMessageTime(timestamp: string | Date): string {
  ...
  // For React Native, just use the dates directly (timezone handling is simpler)
  const saDate = date
  const saNow = now
  ...
  // Full date format - simplified for React Native compatibility
  const day = saDate.getDate()
  const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
  const month = monthNames[saDate.getMonth()]
  const year = saDate.getFullYear() !== saNow.getFullYear() ? ` ${saDate.getFullYear()}` : ''
...
export function formatFullMessageTime(timestamp: string | Date): string {
  ...
  // For React Native, use dates directly
  const saDate = date
  const saNow = now
  ...
  if (saDate.getDate() === saNow.getDate() && 
    ...
    return formatTime(saDate)
  }

  // Yesterday
  if (diffDays === 1) {
    return `Yesterday ${formatTime(saDate)}`
  }
  ...
  const month = monthNames[saDate.getMonth()]
  const year = saDate.getFullYear() !== saNow.getFullYear() ? ` ${saDate.getFullYear()}` : ''
  return `${day} ${month}${year} ${formatTime(saDate)}`
}
